import React from 'react'

function About() {
  return (<>
    <div className='about'>
        <img src='https://scholarlykitchen.sspnet.org/wp-content/uploads/2018/11/iStock-857146092.jpg' className='image'/>

        <div className='box'>

            <div>
            <h2 className='title'>who we are?</h2>
            </div>

            <div>
            <p className='content'>
                ברוכים הבאים לזירת הנדלן - The real estate arena
            </p>
            <p className='content'>
            תיווך דירות למכירה והשכרה / נדל"ן מסחרי / מגרשים למכירה / דירות לתקופות קצרות
            </p>
            </div>

            <div>
            <p className='content'>
            זירת הנדלן גאה לשווק נכסים ומגרשים ברמה גבוהה למכירה והשכרה בכל הארץ. אנו מאמינים כי עסקת נדל"ן היא בין ההחלטות המשמעותיות והמשפיעות שתקבלו בחייכם ואנחנו יודעים כיצד לתרגם את החלום שלכם לעסקה המתאימה והמדויקת ביותר עבורכם. השותפים שלנו יעניקו לכם ייעוץ וליווי לאורך כל תהליך הפרויקט, תוך הקפדה על רמת אמינות ושקיפות גבוהות ומקצועיות בלתי מתפשרת, כך שתוכלו להרגיש שקטים, רגועים ושבעי רצון.
            </p>
            </div>
        </div>
    </div>
  </>)
}

export default About